...
class TaskStateTransitionInteractor:
    ...
    def update_task_state_wrapper(
        ...
        try:
            task_details_dto = self.update_task_state(
        ...
        except MandatoryChecklistNotChecked:
            project_presenter.raise_checklist_not_satisfied_exception()

        response = task_presenter.get_task_details_response(task_details_dto)
        return response

    def update_task_state(self, update_task_state_input_data: Dict):
        ...
        if is_mandatory_checklist_not_checked:
            raise MandatoryChecklistNotChecked()
