...
from project_management_portal.exceptions import InvalidLimitValue,\
                                                 InvalidOffsetValue
...
from project_management_portal.dtos import ProjectsDetailsDto
...
class GetProjectsInteractor(ValidationsMixin):
    ...
    def get_projects_wrapper(self,  user_id: int, offset: int, limit: int,
        ...
        try:
            total_projects_count, projects_details_dto = \
        ...
        user_projects_dict = project_presenter.get_projects_response(
            total_projects_count=total_projects_count,
            projects_details_dto=projects_details_dto
            )

        return user_projects_dict
        ...
        if is_admin:
            ...
            project_dtos = self\
                .project_storage.get_projects_for_user(user_id=user_id,
                                                       offset=offset,
                                                       limit=limit)
        projects_details_dto = self._prepare_projects_details_dto(project_dtos)

        return total_projects_count, projects_details_dto

    @staticmethod
    def _prepare_projects_details_dto(project_dtos):
        ...
        creators_ids = [
            project_dto.created_by_id
            for project_dto in project_dtos
            ]

        user_ids = developers_ids + creators_ids
